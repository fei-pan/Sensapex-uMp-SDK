...
#define LIBUMP_VERSION_STR    "v0.702"
#define LIBUMP_COPYRIGHT      "Copyright (c) Sensapex 2017. All rights reserved"
...
{
    return dev == SMCP1_ALL_CUS || (dev > 0 && dev <= SMCP1_ALL_MANIPULATORS && dev <= LIBUMP_MAX_MANIPULATORS);
}
...
//%m is GNU C specific: sprintf(hndl->errorstr_buffer,"select failed - %m (%d)", hndl->last_error);
...
static int ump_send(ump_state *hndl, const int dev, const unsigned char *data, int dataSize)
{
    int ret;
    IPADDR to;
    if(dev > 0 && dev < LIBUMP_MAX_MANIPULATORS && hndl->addresses[dev].sin_port)
...
int ump_goto_position(ump_state *hndl, const int x, const int y, const int z, const int w, const int speed)
{
    if(!hndl)
        return set_last_error(hndl, LIBUMP_NOT_OPEN);
    return ump_goto_position_ext(hndl, hndl->last_device_sent, x, y, z, w, speed, 0);
}

static int is_invalid_pos(int pos)
{
    if((pos < 0 || pos > LIBUMP_MAX_POSITION*1000) && pos != SMCP1_ARG_UNDEF)
        return LIBUMP_INVALID_ARG;
    return 0;
}

int ump_goto_position_ext(ump_state *hndl, const int dev, const int x, const int y,
                          const int z, const int w, const int speed, const int mode)
{
    int ret, args[6], argc = 0;
    if(!hndl)
        return set_last_error(hndl, LIBUMP_NOT_OPEN);
    if(is_invalid_dev(dev))
        return set_last_error(hndl, LIBUMP_INVALID_DEV);
...
static int ump_recv(ump_state *hndl, ump_message *msg)
{
    IPADDR from;
    int receiver_id, sender_id, message_id, type, sub_blocks, data_size, data_type, options, status, ret = 0;
    smcp1_frame *header = (smcp1_frame *)msg;
    smcp1_subblock_header *sub_block = (smcp1_subblock_header *)((unsigned char*)msg + SMCP1_FRAME_SIZE);
    int32_t *data_ptr = (int32_t*)(msg) + (SMCP1_FRAME_SIZE + SMCP1_SUB_BLOCK_HEADER_SIZE)/sizeof(int32_t);
    ...
    if(receiver_id != SMCP1_ALL_CUS && receiver_id != SMCP1_ALL_PCS &&
            receiver_id != (SMCP1_ALL_CUS|SMCP1_ALL_PCS) && receiver_id != hndl->own_id)
        return set_last_error(hndl, LIBUMP_INVALID_DEV);
    ...
    if(sub_blocks > 0 && options & SMCP1_OPT_NOTIFY && is_valid_dev(sender_id))
    {
        ...
        case SMCP1_NOTIFY_POSITION_CHANGED:
            if(data_size > 0 && (data_type == SMCP1_DATA_INT32 || data_type == SMCP1_DATA_UINT32))
            {
                ...
                int time_step_us = ump_update_position_cache_time(hndl, sender_id);
                // X axis
                u_long pos_nm = ntohl(*data_ptr++);
        ...
        case SMCP1_NOTIFY_GOTO_POS_COMPLETED:
            if(data_size > 0 && (data_type == SMCP1_DATA_INT32 || data_type == SMCP1_DATA_UINT32))
            {
                ...
                {
                    hndl->drive_status[sender_id] = status?LIBUMP_POS_DRIVE_FAILED:LIBUMP_POS_DRIVE_COMPLETED;
    ...
    if(!(options&SMCP1_OPT_REQ)) // response to our own request
    {
        // Response to our own request

        // TODO SMCPv1 emu does not currently use message id from the request for response,
        // to be checked out if real MCU does
        // if(message_id == hndl->message_id)
        {
...
static int ump_send_msg(ump_state *hndl, const int dev, const int cmd,
                        const int argc, const int *argv, const int respc, int *respv)
{
    ...
    if(argc)
    {
        // Initially supporting only one data block per request
...
int ump_get_param(ump_state *hndl, const int dev, const int param_id, int *value)
{
    ...
    if((ret = ump_send_msg(hndl, dev, SMCP1_GET_PARAMETER, 1, &param_id, 2, resp)) < 0)
        return ret;
...
int ump_cmd_get_axis_angle(ump_state *hndl, const int dev, const int actuator, const int layer) {
    ...
    args[argc++] = layer;
      ...
      if((ump_send_msg(hndl, dev, SMCP1_CMD_GET_AXIS_ANGLE, argc, args, 1, &resp[0])) < 0)
	         return 0;
...
int ump_get_feature(ump_state *hndl, const int dev, const int feature_id)
{
    ...
    if((ret = ump_send_msg(hndl, dev, SMCP1_GET_FEATURE, 1, &feature_id, 2, resp)) < 0)
        return ret;
                         ...
                         int *version, const int size)
{
    ...
    if(is_invalid_dev(dev))
        return set_last_error(hndl, LIBUMP_INVALID_DEV);
    return ump_send_msg(hndl, dev, SMCP1_GET_VERSION, 0, NULL, size, version);
}
                         ...
                         int *x, int *y, int *z, int *w, unsigned long long *elapsedptr)
{
    ...
    // Obtain values from the cache if new enough


    if(((hndl->last_positions[dev].x != LIBUMP_ARG_UNDEF) && (hndl->last_positions[dev].y != LIBUMP_ARG_UNDEF) &&
        (hndl->last_positions[dev].z != LIBUMP_ARG_UNDEF) && (hndl->last_positions[dev].w != LIBUMP_ARG_UNDEF)) &&
        (elapsed < (unsigned long)time_limit || time_limit == LIBUMP_TIMELIMIT_CACHE_ONLY) && time_limit != LIBUMP_TIMELIMIT_DISABLED)
    {
    ...
    memset(resp, 0, sizeof(resp));
    start = get_timestamp_ms();
    if((ret = ump_send_msg(hndl, dev, SMCP_GET_POSITIONS, 0, NULL, 4, resp)) > 0)
    {
...
int ump_cu_read_version(ump_state *hndl, int *version, const int size)
{
    ...
    if(!version || size < 1)
        return set_last_error(hndl, LIBUMP_INVALID_ARG);
    return ump_send_msg(hndl, SMCP1_ALL_CUS, SMCP1_GET_VERSION, 0, NULL, size, version);
}

int ump_get_broadcasters(ump_state *hndl,int *devs,  int *count)
{
    ...
    for ( int i=0; i < LIBUMP_MAX_MANIPULATORS; i++) {
...
int ump_clear_broadcasters(ump_state *hndl)
{
    if(!hndl)
        return set_last_error(hndl, LIBUMP_NOT_OPEN);
